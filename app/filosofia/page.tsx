  ...
  return (
    <div className="min-h-screen bg-notaria-light pt-32 pb-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-5xl relative z-10">
        ...
        >
          <span className="text-notaria-gold font-semibold tracking-widest uppercase text-sm mb-4 block">Nuestra Esencia</span>
          <h1 className="font-serif text-5xl md:text-6xl lg:text-7xl text-notaria-dark font-bold mb-8">
            Más Allá de los <span className="text-notaria-gold italic">Modelos</span>
          </h1>
          <p className="text-xl md:text-2xl text-notaria-dark/70 font-light max-w-3xl mx-auto leading-relaxed">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-notaria-gold via-yellow-500 to-notaria-gold rounded-3xl opacity-10 blur-xl"></div>
          <div className="bg-white rounded-3xl p-8 md:p-14 shadow-xl border border-notaria-gold/20 relative z-10 flex flex-col md:flex-row items-center gap-10">
            <div className="w-24 h-24 rounded-full bg-notaria-gold/10 flex items-center justify-center flex-shrink-0">
              <Lightbulb className="w-12 h-12 text-notaria-gold" />
            </div>
            <div>
              <blockquote className="font-serif text-2xl md:text-3xl text-notaria-dark italic leading-relaxed">
        ...
        <div className="mb-24">
          <div className="text-center mb-16">
            <h2 className="font-serif text-3xl md:text-4xl text-notaria-dark font-bold">Pilares de la Notaría 174</h2>
            <div className="w-16 h-1 bg-notaria-gold mx-auto mt-6"></div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {pilares.map((pilar, idx) => (
              <motion.div
                ...
                className="bg-white p-8 rounded-3xl shadow-sm border border-transparent hover:border-notaria-gold/30 hover:shadow-lg transition-all duration-300 group"
              >
                <div className="flex items-start gap-6">
                  <div className="w-16 h-16 rounded-2xl bg-notaria-gold/10 flex items-center justify-center text-notaria-gold flex-shrink-0 group-hover:bg-notaria-gold group-hover:text-white transition-colors duration-300">
                    <pilar.icon className="w-8 h-8" />
                  </div>
                  <div>
                    <h3 className="font-serif text-2xl font-semibold text-notaria-dark mb-3">{pilar.title}</h3>
                    <p className="text-notaria-dark/70 leading-relaxed">{pilar.desc}</p>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <motion.div 
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
          className="prose prose-lg md:prose-xl prose-stone mx-auto text-notaria-dark/80 text-center bg-notaria-gold/5 p-10 rounded-3xl border border-notaria-gold/10"
        >
          <Handshake className="w-12 h-12 text-notaria-gold mx-auto mb-6 opacity-50" />
          <p className="mb-6">
